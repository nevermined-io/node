...
describe('Info', () => {
  ...
  it('bad method', async () => {
    const response = await request(app.getHttpServer()).post(`/`).send({message: 'msg', method: 'foo'});
    expect(response.statusCode).toBe(400);
    expect((response.error as any).text).toContain('Only PSK-ECDSA or PSK-RSA encryption allowed');
  });
  it('correct call', async () => {
    const response = await request(app.getHttpServer()).post(`/`).send({message: 'msg', method: 'PSK-RSA'});
